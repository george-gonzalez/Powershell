...
$licServerPort ="5053"
$rlmutil = C:\Temp\rlmutil.exe rlmstat -c $licServerPort@$licServer -i eonsoftw
...
$outputData = ($rlmutil | Select-String -Pattern "vuee" | ForEach-Object {
    $_ -replace '1/0 at ',"" `
...
$csvData = import-csv $outputFile -Header application, username, computer, checkoutdate, checkouttime

#Total Count of licenses used

$totalUsed = $outputData | Measure-Object -Line | Select-Object -ExpandProperty Lines
